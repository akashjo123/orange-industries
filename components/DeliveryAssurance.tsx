  ...
  return (
    <section className="py-24 bg-zinc-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-16">
          {companyData.procurementControls.map((item, idx) => (
            <motion.div
              ...
              className="bg-zinc-900 p-6 space-y-3 hover:shadow-lg transition-all duration-300 relative group shadow-md rounded-xl"
            >
              <span className="font-mono text-xl font-bold text-brand-orange">
                {item.number}
              </span>
              <h3 className="text-sm font-bold text-zinc-50 uppercase font-mono">
                {item.title}
              </h3>
        ...
        <div className="bg-zinc-900 p-8 sm:p-12 shadow-md space-y-8 rounded-2xl">
          <div className="space-y-2 pb-2">
            <span className="font-mono text-xs font-bold text-brand-orange uppercase tracking-widest block">
              QUALITY, SAFETY + COMPLIANCE
            </span>
            <h3 className="text-2xl sm:text-3xl font-extrabold text-zinc-50">
              Documentation should make procurement easier
            </h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 font-mono text-xs">
            <div className="bg-zinc-950 p-5 space-y-2 rounded-xl">
              <span className="text-brand-orange font-bold uppercase block">QUALITY CONTROL</span>
              <p className="text-zinc-400 leading-relaxed">{companyData.compliance.qualityControl}</p>
            </div>

            <div className="bg-zinc-950 p-5 space-y-2 rounded-xl">
              <span className="text-brand-orange font-bold uppercase block">SITE SAFETY</span>
              <p className="text-zinc-400 leading-relaxed">{companyData.compliance.siteSafety}</p>
            </div>

            <div className="bg-zinc-950 p-5 space-y-2 rounded-xl">
              <span className="text-brand-orange font-bold uppercase block">COMMERCIAL ASSURANCE</span>
              <p className="text-zinc-400 leading-relaxed">{companyData.compliance.commercialAssurance}</p>
            </div>

            <div className="bg-zinc-950 p-5 space-y-2 rounded-xl">
              <span className="text-brand-orange font-bold uppercase block">ENVIRONMENTAL CONTROL</span>
              <p className="text-zinc-400 leading-relaxed">{companyData.compliance.environmentalControl}</p>
            </div>
          </div>
        </div>
