    ...
    <section id="work" className="py-24 bg-zinc-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <Link
            href="/portfolio"
            className="inline-flex items-center gap-2 text-brand-orange font-mono text-xs uppercase tracking-wider hover:text-zinc-50 transition-colors font-bold"
          >
          ...
          {projects.slice(0, 3).map((project, idx) => (
            ...
            >
              <Link
                ...
                className="group block bg-zinc-900 overflow-hidden transition-all duration-300 relative shadow-sm hover:shadow-md h-full flex flex-col justify-between rounded-sm"
              >
                <div>
                  ...
                  <div className="relative h-64 w-full overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 flex items-center gap-2">
                      <span className="bg-zinc-900/90 backdrop-blur px-3 py-1 text-[11px] font-mono text-zinc-50 uppercase tracking-wider font-bold shadow-xs">
                        {project.client}
                      </span>
                    </div>

                    {/* Top Right Arrow */}
                    <div className="absolute top-4 right-4 w-9 h-9 bg-zinc-900/90 backdrop-blur flex items-center justify-center text-zinc-50 group-hover:bg-brand-orange group-hover:text-white transition-all duration-300 shadow-xs">
                      <ArrowUpRight className="w-4 h-4" />
                    </div>
                  </div>
                  ...
                  <div className="p-6 space-y-4 bg-zinc-900">
                    ...
                    <div className="pt-2 space-y-1">
                      <span className="font-mono text-[10px] text-zinc-500 uppercase tracking-wider font-semibold block">
                        SCOPE INCLUSIONS:
                      </span>
                      {project.scopeList.map((item, i) => (
                        <div key={i} className="flex items-center gap-1.5 text-xs font-mono text-zinc-300">
                ...
                <div className="p-6 pt-0 bg-zinc-900">
        ...
        <div className="mt-16 bg-zinc-900 p-8 space-y-6 shadow-sm rounded-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 gap-4">
            ...
            <span className="font-mono text-xs text-zinc-500 font-semibold">VERIFIED EXECUTION</span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {selectedPortfolioItems.map((item, idx) => (
              <div
                key={idx}
                className="bg-zinc-800 p-4 text-center font-mono text-xs font-bold text-zinc-100 hover:text-brand-orange transition-colors shadow-xs rounded-sm"
              >
                {item}
              </div>
            ))}
